...
class ImageWithExif extends StatefulWidget {
  @override
  State<StatefulWidget> createState() => _ImageWithExif();
}

class _ImageWithExif extends State<ImageWithExif> {
  File _image;
  ...
  Future<String> takePicture(CameraController controller) async {
    ...
    final String dirPath = '${extDir.path}/Downloads/flutter_test';
  ...
  Future<Widget> getImageFromCamera(BuildContext context) async {
    Widget res;
    if (_image == null) {
      res = Text('No image selected.', style: TextStyle(color: Colors.white));
    } else {
